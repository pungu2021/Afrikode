...
date_default_timezone_set('Africa/Kinshasa');
 function afrikode($class){
   ...
   if(isset($_POST["publier"])){
       $mod="";
    $objet->pdf_code_source($_POST["desc"],$_POST["poid"],$_FILES["fichier"]);
   }
   if(isset($_GET["id"])){
    $mod=$objet->pdf_source_rec($_GET["id"]);
   }
   if(isset($_POST["modifier_valide"])){
    $objet->pdf_source_modifier($_POST["desc"],$_POST["poid"],$_FILES["fichier"],$_GET["id"]);
   }
   if(isset($_GET["para"])&& !empty($_GET["para"])){
    $objet->pdf_source_suppression($_GET["id"]);
   }
   $recuperation=$objet->pdf_source_recuperation();
